...
get_variable_measures_from_acmt <- function (city_hall_to_lat_long_list, radius_vector, year, names_of_variable_to_get, external_data_name_to_info_list=NULL) {
  city_hall_to_distance_to_variable_to_measures_list <- vector(mode="list", length=length(city_hall_to_lat_long_list))
  names(city_hall_to_distance_to_variable_to_measures_list) <- names(city_hall_to_lat_long_list)

  for(city_hall in names(city_hall_to_distance_to_variable_to_measures_list)) {
    distance_to_variable_to_measures_list <- vector(mode="list", length=length(radius_vector))
    names(distance_to_variable_to_measures_list) <- as.character(radius_vector)
    for (distance in radius_vector) {
      print(city_hall)
      print(distance)
      # get lat/long
      ...
      environmental_measures <- get_acmt_standard_array(long=longitude, lat=latitude, radius_meters = distance, year=year, external_data_name_to_info_list=external_data_name_to_info_list)
      ...
      #distance_to_variable_to_measures_list[[which(distance == radius_vector)]] <- variable_to_measures_list
      distance_to_variable_to_measures_list[[as.character(distance)]] <- variable_to_measures_list
    }
    city_hall_to_distance_to_variable_to_measures_list[[city_hall]] <- distance_to_variable_to_measures_list
  }
  return(city_hall_to_distance_to_variable_to_measures_list)
}
```
...
year <- 2017
names_of_variable_to_get <- c("total_pop_count")

start_time_get_variable_measures_from_acmt <- Sys.time()
city_hall_to_distance_to_variable_to_measures_list <- get_variable_measures_from_acmt(city_hall_to_lat_long_list=city_hall_to_lat_long_list, radius_vector=radius_vector, year=year, names_of_variable_to_get=names_of_variable_to_get)
end_time_get_variable_measures_from_acmt <- Sys.time()
```

```{r}
print(city_hall_to_distance_to_variable_to_measures_list[1:2])
...
```{r, results = 'hide'}
add_density_measures <- function(city_hall_to_distance_to_variable_to_measures_list) {
  city_hall_to_distance_to_variable_to_measures_with_population_density_list <- city_hall_to_distance_to_variable_to_measures_list  # R's way of making a deep copy
  for (city_hall in names(city_hall_to_distance_to_variable_to_measures_with_population_density_list)){
    for(distance_character in names(city_hall_to_distance_to_variable_to_measures_with_population_density_list[[city_hall]])){
      for(variable in names(city_hall_to_distance_to_variable_to_measures_with_population_density_list[[city_hall]][[distance_character]])) {
        if(variable == "total_pop_count"){  # add other "if" statements if you want to compute other measures
          total_pop_count <- city_hall_to_distance_to_variable_to_measures_with_population_density_list[[city_hall]][[distance_character]][[variable]]
          distance_numeric <- as.numeric(distance_character)
          population_density <-  total_pop_count/(pi*distance_numeric^2)  # distance is radius
          city_hall_to_distance_to_variable_to_measures_with_population_density_list[[city_hall]][[distance_character]][["population_density"]] <- population_density
        }
      }
    }
  }
  return(city_hall_to_distance_to_variable_to_measures_with_population_density_list)
}
```

```{r, results = 'hide'}
city_hall_to_distance_to_variable_to_measures_with_population_density_list <- add_density_measures(city_hall_to_distance_to_variable_to_measures_list)
```

```{r}
print(city_hall_to_distance_to_variable_to_measures_with_population_density_list[1:2])
```
...
```{r, results = 'hide'}
convert_to_dataframe_for_plotting <- function (city_hall_to_distance_to_variable_to_measures_with_population_density_list) {
  city_hall_vector <- c()
  radius_vector <- c()
  variable_vector <- c()
  value_vector <- c()
  ...
  for(city_hall in names(city_hall_to_distance_to_variable_to_measures_with_population_density_list)){
    for(distance_character in names(city_hall_to_distance_to_variable_to_measures_with_population_density_list[[city_hall]])){
      for(variable in names(city_hall_to_distance_to_variable_to_measures_with_population_density_list[[city_hall]][[distance_character]])){
        ...
        radius_vector <- c(radius_vector, as.numeric(distance_character))
        variable_vector <- c(variable_vector, variable)
        value_vector <- c(value_vector, city_hall_to_distance_to_variable_to_measures_with_population_density_list[[city_hall]][[distance_character]][[variable]])
      }
    }
  }

  dataframe_for_plotting <- data.frame(city_hall_vector, radius_vector, variable_vector, value_vector, stringsAsFactors=FALSE)
  names(dataframe_for_plotting) <- c("city_hall", "distance", "variable", "value")
  return(dataframe_for_plotting)
}
```

```{r, results = 'hide'}
dataframe_for_plotting <- convert_to_dataframe_for_plotting(city_hall_to_distance_to_variable_to_measures_with_population_density_list)
```
...
```{r}
ggplot(dataframe_for_plotting[dataframe_for_plotting$variable == "population_density", ], aes(x=distance)) +
...
## Results
We see that New York has a much higher population density, and its density does not reduce much over distances.

```{r, include=FALSE}
##Lastly, we ensure that you are reproducing the results the same as what's on other platforms.
## tests
library(testthat)
expect_equal(filter(dataframe_for_plotting[dataframe_for_plotting$variable == "population_density", ], city_hall=="seattle_city_hall", distance==2000)$value, 0.005281557)
expect_equal(filter(dataframe_for_plotting[dataframe_for_plotting$variable == "population_density", ], city_hall=="los_angeles_city_hall", distance==3000)$value, 0.005563233)
expect_equal(filter(dataframe_for_plotting[dataframe_for_plotting$variable == "population_density", ], city_hall=="chicago_city_hall", distance==1000)$value, 0.009057937)
expect_equal(filter(dataframe_for_plotting[dataframe_for_plotting$variable == "population_density", ], city_hall=="boston_city_hall", distance==5000)$value, 0.005571173)
```
...
dir.create(
  "~/workspace/ACMT_shiny/data_pull_measures",
  recursive = TRUE,
  showWarnings = FALSE
)

dir.create(
  "~/workspace/ACMT_shiny/data_pull_summaries",
  recursive = TRUE,
  showWarnings = FALSE
)
...
saveData <- function(data, fileName) {
  ...
  write.csv(
    x = data,
    file = file.path("~/workspace/ACMT_shiny", fileName), 
...
pull_county_geoid<-function(dataset, year=2019){
  ...
  write.csv(county_dataset, '~/workspace/ACMT_shinydata_pull_measures/dataset_county.csv')
}

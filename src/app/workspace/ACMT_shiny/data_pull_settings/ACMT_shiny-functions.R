...
measures.list<-c('<a href = "https://www.census.gov/programs-surveys/acs/about.html" target = "new" >The American Community Survey</a>',
                 ...
                 #'<a href = "https://appliedgeographic.com/crimerisk/" target = "new" >Applied Geographic Solutions CrimeRisk Data</a>', ##ACMT_shiny specific - not publically available
...
if(dir.exists('~/workspace/ACMT_shiny/data_pull_measures')==FALSE){
  dir.create('~/workspace/ACMT_shiny/data_pull_measures')
}

## create data pull summaries folder if it doesn't exist: 
if(dir.exists('~/workspace/ACMT_shiny/data_pull_summaries')==FALSE){
  dir.create('~/workspace/ACMT_shiny/data_pull_summaries')
}

### save data frame to ACMT_shiny Folder ####

outputDir <- "~/workspace/ACMT_shiny"
...
### Load dataset from ACMT_shiny folder ####
...
pull_county_geoid<-function(dataset, year=2019){
  ...
  write.csv(county_dataset, '~/workspace/ACMT_shiny/data_pull_measures/dataset_county.csv')
}
...
measure.directory<-'~/workspace/ACMT_shiny/data_pull_measures/'

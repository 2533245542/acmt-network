...
geocode <- function(address) {
  ...
  if (is.na(latitude) && is.na(longitude)) {
    ...
    print(address)
    print("The state of this address is not loaded into database")
    return(list(latitude=NaN, longitude=NaN, rating=NaN))
  }
...
shp_preprocess <- function (shp_directory){
  #"external_data/ParkServe_shp/Shapefiles_08062021/ParkServe_shp_DataShare_08062021/ParkServe_Parks.shp"
...
check_geocode <- function(lat, long, address, rate){
  side_len = 0.007
  ...
  map <- ggmap(get_stamenmap(bbox, zoom = 15)) +
    ...
    ggtitle(paste(address, ", Rate =", rate, sep = ""))
  return(map)
}

...
download_file_park <- function () {  # download the external dataset and give it a name (will use it in creating external_data_name_to_info_list)
  download.file(url = "https://parkserve.tpl.org/downloads/ParkServe_Shapefiles_05042022.zip?_ga=2.103216521.887440371.1664905337-1364699585.1664905337", destfile = "Inspace/external_data/ParkServe_shp.zip")
...
process_file_park <- function () {  # unzip the downloaded file and save the target data layer as csv file)
  unzip("Inspace/external_data/ParkServe_shp.zip", exdir="Inspace/external_data")
}

prepare_park_data<-function() {
  if(file.exists("Inspace/external_data/ParkServe_Shapefiles_05042022/ParkServe_Parks.shp")==FALSE){
download_file_park()
process_file_park()
  }
  if(exists('park_shp')==FALSE){
    
park_shp <- shp_preprocess(shp_directory = "Inspace/external_data/ParkServe_Shapefiles_05042022/ParkServe_Parks.shp")
